...
sub write_vmware_config {
  ...
  if (vmware_product() eq 'ws') {
    ...
    $config->set('product.version', '9.6.2');
  }
  $config->set('product.buildNumber', '1688356');
...
sub mod_pre_install_check {
  ...
  if ($modStatus eq 'installed_by_other') {
    if (defined $clobberKMod and $clobberKMod eq 'yes') {
      ...
      unless ($mod eq 'vmci' || $mod eq 'vsock' || $deprecated_310) {
		 print wrap("Use the flag --clobber-kernel-modules=$mod " .
		 "to override.\n\n", 0);
      }
      return 'no';
    }
...
sub get_resolution {
  ...
  if ($xrandrPath ne '' and system("$xrandrPath > /dev/null 2>&1") == 0) {
    my $currentRes = direct_command("$xrandrPath | grep \\*"); # get the line with the resolution
